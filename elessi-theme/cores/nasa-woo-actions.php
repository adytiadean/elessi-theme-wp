...
if (!function_exists('elessi_remove_action_woo')) :
    function elessi_remove_action_woo() {
        ...
        /**
         * Remove Yith wishlist btn in detail product
         */
        // if (NASA_WISHLIST_ENABLE) {
...
if (!function_exists('elessi_add_action_woo')) :
    function elessi_add_action_woo() {
        ...
        // For Product content
        // add_action('woocommerce_before_shop_loop_item', 'elessi_loop_countdown');
...
if (!function_exists('elessi_custom_woocommerce_post_class')) :
    function elessi_custom_woocommerce_post_class($classes) {
        global $nasa_opt, $product, $nasa_animated_products, $nasa_time_sale;
    ...
    function elessi_get_childs_category($term = null, $instance = array()) {
        ...
        if (NASA_WOO_ACTIVED){
            ...
            if ($nasa_terms) {
                ...
                if ($show && ($items > $show)) {
                    $content .= '<li class="nasa_show_manual"><a data-show="1" class="nasa-show" href="javascript:void(0);" data-text="' . esc_attr__('- Show less', 'elessi-theme') . '">' . esc_html__('+ Show more', 'elessi-theme') . '</a></li>';
                }
...
if (!function_exists('elessi_bundle_in_list')) :
    function elessi_bundle_in_list($combo_show_type) {
        ...
        ?>
        <a href="javascript:void(0);" class="btn-combo-link btn-link gift-icon nasa-tip" data-prod="<?php echo (int) $product->get_id(); ?>" data-tip="<?php esc_attr_e('Promotion Gifts', 'elessi-theme'); ?>" title="<?php esc_attr_e('Promotion Gifts', 'elessi-theme'); ?>" data-show_type="<?php echo esc_attr($combo_show_type); ?>">
...
if (!function_exists('elessi_add_compare_in_list')) :
    function elessi_add_compare_in_list() {
        ...
        ?>
        <a href="javascript:void(0);" class="<?php echo esc_attr($class_btn); ?>" data-prod="<?php echo (int) $productId; ?>" data-icon-text="<?php esc_attr_e('Compare', 'elessi-theme'); ?>" title="<?php esc_attr_e('Compare', 'elessi-theme'); ?>">
            <?php echo $icon; ?>
        </a>
...
if (!function_exists('elessi_add_compare_in_detail')) :
    function elessi_add_compare_in_detail() {
        ...
        ?>
        <a href="javascript:void(0);" class="<?php echo esc_attr($class_btn); ?>" data-prod="<?php echo (int) $productId; ?>" data-tip="<?php esc_attr_e('Compare', 'elessi-theme'); ?>" title="<?php esc_attr_e('Compare', 'elessi-theme'); ?>">
            <?php echo $icon; ?>
...
if (!function_exists('elessi_add_custom_sale_flash')) :
    function elessi_add_custom_sale_flash() {
        ...
        if ($product->is_on_sale()) :
            ...
            $product_type = $product->get_type();
            if ($product_type == 'variable') :
                $badges .= '<span class="badge sale-label sale-variable">' . esc_html__('SALE', 'elessi-theme') . '</span>';
                ...
                if (is_numeric($sales_price)) :
                    ...
                    $maximumper = $maximumper - (2*$maximumper);
                    $badges .= '<span class="badge sale-label">' . sprintf('%s', $maximumper . '%') . '</span>';
                endif;
            endif;
        ...
        if ($stock_status == "outofstock"):
            $badges .= '<span class="badge out-of-stock-label">' . esc_html__('Sold Out', 'elessi-theme') . '</span>';
        endif;
