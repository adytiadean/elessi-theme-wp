...
class Elessi_DF_Page_Importer {
    ...
    public static function nasa_render_page_data($type, $data) {
        ...
        $default_data = $type == 'wpb' ? self::nasa_wpb_default_page() : self::nasa_elm_default_page();
    ...
    public static function nasa_wpb_page_data($page) {
        $file = ELESSI_ADMIN_PATH . 'importer/data-import/wpb-pages/nasa_' . $page . '.php';
        if (!is_file($file)) {
            return false;
        }
    ...
    public static function nasa_elm_page_data($page) {
        $file = ELESSI_ADMIN_PATH . 'importer/data-import/elm-pages/nasa_' . $page . '.php';
        if (!is_file($file)) {
            return false;
        }
    ...
    public static function nasa_push_data_from_file($type, $page) {
        ...
        $data = $type == 'wpb' ? self::nasa_wpb_page_data($page) : self::nasa_elm_page_data($page);
        ...
        if ($insert_id) {
            ...
            if (!isset($nasa_front_page)) {
                $GLOBALS['nasa_front_page'] = $insert_id;
                update_option('page_on_front', $insert_id);
            }

            $build_css = self::nasa_build_page_css($insert_id, $data_push);
        }
    }
}
