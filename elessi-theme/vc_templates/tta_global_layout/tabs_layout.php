...
if (WPBakeryShortCode_VC_Tta_Section::$section_info):
    ...
    foreach (WPBakeryShortCode_VC_Tta_Section::$section_info as $k => $v):
        ...
        $content .= '<a href="javascript:void(0);" data-index="nasa-section-' . esc_attr($v['tab_id']) . '" class="' . esc_attr($class_a_click) . '"' . (!empty($tab_color_text) ? ' style="' . implode(';', $tab_color_text) . '"' : '') . '>' . $title . '</a></li>';
    endforeach;
